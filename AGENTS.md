...
Continue this source; do not restart, redesign, replace the PDF engine, or reimplement completed UI changes. Keep source IDs, imported content ownership, IndexedDB name/version and the existing backup format. State saves are reading-session checkpoints, not content/library rollback. Preserve newer notes/PDFs and separate reading-position bookmarks.
