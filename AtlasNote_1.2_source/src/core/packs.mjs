...
export async function readWorkspace(files,schemas,{publicOnly=false,reviewed={},allowUnresolved=false}={}){
 ...
 let documents=[];if(files.has(prefix+dir+'atlas-documents.json')){const index=read(dir+'atlas-documents.json');validateSchema(index,schemas.documents,'documents');if(index.packId!==manifest.id)throw Error('Document index owner mismatch');documents=index.documents.map(d=>({...d,packId:manifest.id,assetKey:d.source.kind==='pack-file'?manifest.id+'@'+manifest.version+'/'+d.source.path:undefined}));
  for(const d of documents){const referenceMetadata=d.rights.status==='reference-only'&&d.source.kind==='https'&&d.source.url?.startsWith('https://')&&!d.assetKey&&!manifest.assets.length&&reviewed[manifest.id]?.metadataOnlyReferences===true;
   if(publicOnly&&(d.visibility!=='public'||(!['author-created','permission'].includes(d.rights.status)&&!referenceMetadata)))throw Error('Unreviewed PDF in public build');if(d.source.kind==='pack-file'){safePath(d.source.path);const asset=assets.find(a=>a.key===d.assetKey);if(!asset||asset.sha256!==d.sha256||asset.bytes.length!==d.bytes)throw Error('PDF bytes/hash mismatch: '+d.id);}}
 }
