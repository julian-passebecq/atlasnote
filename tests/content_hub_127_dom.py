...
with sync_playwright() as pw:
 ...
 def capture(mode,texts,subject=None,attach=False,due=None):
  click('Quick Capture',p.locator('.workspace-dock'));click(mode,p.locator('dialog'))
 ...
 def dashboard():close_panels(p);click('Open Dashboard');expect(p.locator('.dashboard-page')).to_be_visible()
 def basic():
  reset();expect(p.locator('.dashboard-cell')).to_have_count(25);assert p.locator('.dashboard-row-label').all_text_contents()==['Inbox','To-do','Quick notes','Bookmarks','Read later'];assert p.locator('.content-type-selector button').count()==5;assert p.locator('.subject-selector button').all_text_contents()==['IT','Cloud','Job','KPI','Norsk'];assert p.locator('.sidebar-navigation button').count()==6;assert p.locator('.project-tree .workspace-slots').count()==0;expect(p.locator('.workspace-dock .workspace-slots button')).to_have_count(5);shot('dashboard-empty');return {'cells':25,'subjects':5,'types':5,'workspaceCap':5}
 check('Five-by-five Dashboard and separate content/subject/workspace axes',basic)
 def article():
 ...
 def bad_import():
  click('Article content');button('Add article').last.click();p.get_by_role('checkbox',name='Edit canonical JSON',exact=True).check();before=state()['overlays'];bad={**ARTICLE,'id':'unsafe-source','blocks':[{'id':'bad','type':'html','html':'<iframe src="https://example.invalid/"></iframe>'}]};bad.pop('text',None)
 ...
 def batch_limit():
  click('Quick Capture',p.locator('.workspace-dock'));click('Note',p.locator('dialog'))
  for i in range(5):
   if i:click('Add row')
 ...
 def board():
  before=session()['panes'];dashboard();assert session()['panes']==before;cell=p.get_by_role('cell',name='Cloud Quick notes',exact=True);expect(cell.locator('.dashboard-card')).to_have_count(3);click('Show more (5)',cell);expect(cell.locator('.dashboard-card')).to_have_count(5);assert p.locator('.dashboard-unclassified').is_visible();expect(p.get_by_role('cell',name='Norsk Inbox',exact=True)).to_contain_text(ARTICLE['title']);shot('dashboard-filled');click('Complete Review shuffles tomorrow');assert next(i for i in state()['personal']['dashboardItems'] if i['text']=='Review shuffles tomorrow')['status']=='done';p.get_by_role('checkbox',name='Include completed',exact=True).check();click('Reopen Review shuffles tomorrow');assert next(i for i in state()['personal']['dashboardItems'] if i['text']=='Review shuffles tomorrow')['status']=='open';click('Source for Review shuffles tomorrow');expect(p.locator('.qcm-reader')).to_have_attribute('data-qcm-question','q2')
 ...
 def article_lifecycle():
  reset();import_source('Article',ARTICLE);click('Article content');click('Edit / details');p.get_by_label('Title',exact=True).fill('Edited native transcript');p.get_by_label('Pasted article or transcript',exact=True).fill('Edited safe local body.');p.get_by_label('Article status',exact=True).select_option('reading');click('Save content');expect(p.locator('.active-pane')).to_contain_text('Edited safe local body.');pg=state()['overlays']['pages'][ARTICLE['id']]['page'];assert pg['article']['status']=='reading';assert pg['article']['updatedAt']>=pg['article']['addedAt'];click('Article content');p.get_by_label('Managed article status',exact=True).select_option('finished');p.wait_for_timeout(100)
  ...
  click('Archive resource');expect(p.locator('.managed-resource')).to_have_count(0);p.get_by_role('checkbox',name='Include archived',exact=True).check();click('Restore resource');assert ARTICLE['id'] not in state()['overlays']['archived'];assert state()['overlays']['pages'][ARTICLE['id']]['page']['article']['status']=='finished';shot('article-manager-edited')
