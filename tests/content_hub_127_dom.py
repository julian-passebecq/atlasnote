...
with sync_playwright() as pw:
 ...
 def qcm_rejection():
  reset();import_source('QCM',QCM);click('QCM content');button('New/import QCM').last.click();p.get_by_role('checkbox',name='Edit canonical JSON',exact=True).check();before=state();bad=json.loads(json.dumps(QCM));bad['id']='unsafe-qcm';bad['questions'][0]['html']='<script>window.UNSAFE_QCM=1</script>'
