...
from browser_support import start_server,launch,open_context
...
try:
 with sync_playwright() as p:
  b=launch(p);page=b.new_page()
  page.goto(url,wait_until='networkidle');page.get_by_role('button',name='Global search',exact=True).click();page.get_by_role('textbox',name='Search all pages and glossary',exact=True).fill('A quiet place');page.locator('.search-result').first.click();open_context(page,'Remarks');page.get_by_role('textbox',name='Personal remarks').fill('ATLAS_REAL_IDB_RELOAD_TEST');page.get_by_text('Saved locally',exact=False).wait_for();page.reload(wait_until='networkidle');open_context(page,'Remarks');assert page.get_by_role('textbox',name='Personal remarks').input_value()=='ATLAS_REAL_IDB_RELOAD_TEST';b.close();result['status']='PASS'
