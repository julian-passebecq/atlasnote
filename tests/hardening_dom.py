...
with sync_playwright() as pw:
    ...
    def keep_left():
        ...
        page.locator('.document-pane').first.get_by_role('button',name='Parallel',exact=True).click()
    ...
    def top_order():
        reset()
        labels=page.locator('.topbar-actions > button').evaluate_all('(xs)=>xs.map(x=>x.getAttribute("aria-label")||x.textContent.trim())')
        assert labels==['Export to AI','Print or Save as PDF','Enter focus mode','Compare in two panes','Show learning flags'],labels
        return labels
    check('Focus immediately precedes Compare in the requested control order',['E01'],top_order)
    ...
    def tabs_state():
        reset('page.atlas.layouts','book');page.get_by_role('button',name='Hide English').click()
        ...
        tree('Read in Norwegian').click(modifiers=['Control']);page.get_by_role('button',name='Parallel',exact=True).click()
        ...
        assert page.get_by_role('button',name='Show English').is_visible();assert page.locator('.book-scroller').is_visible()
    ...
    def remarks():
        reset('audit.page.long',synthetic=True);page.get_by_role('button',name='Remarks',exact=True).click();page.get_by_role('textbox',name='Personal remarks').fill('Remark A exact')
        tree('Parallel language stress note').click();page.get_by_role('textbox',name='Personal remarks').fill('Remark B exact')
    ...
    def context_glossary():
        ...
        page.locator('.context-panel').get_by_role('button',name='Context',exact=True).click()
    ...
    def flags():
        ...
        for flag in ['red','orange','green','gray']:
            page.get_by_role('combobox',name='Learning flag').select_option(flag);assert state('personal.ratings')['page.atlas.welcome']==flag
        page.get_by_role('button',name='Show learning flags').click();assert page.get_by_role('combobox',name='Learning flag').count()==0
        assert state('personal.ratings')['page.atlas.welcome']=='gray'
        page.get_by_role('button',name='Show learning flags').click()
    ...
    def folder_crud():
        ...
        page.get_by_role('button',name='Home',exact=True).click();page.locator('.archive-list > summary').click();page.locator('.archive-row').filter(has_text='First local folder').get_by_role('button',name='Restore',exact=True).click()
    ...
    def root_page_crud():
        ...
        page.get_by_role('button',name='Home',exact=True).click();page.locator('.archive-list > summary').click();page.locator('.archive-row').filter(has_text='Root acceptance page').get_by_role('button',name='Restore',exact=True).click()
    ...
    def outline_jump():
        ...
        page.locator('.context-panel').get_by_role('button',name='Outline',exact=True).click()
    ...
    def pdf_revision_remark():
        ...
        page.locator('.context-panel').get_by_role('button',name='Remarks',exact=True).click()
    ...
    def pane_isolation():
        ...
        left.get_by_role('button',name='Parallel',exact=True).click();right.get_by_role('button',name='Book',exact=True).click();page.wait_for_timeout(300)
    ...
    def bookmark_update():
        ...
        page.get_by_role('button',name='Bookmarks',exact=True).click();page.locator('.bookmark-card > button').first.click();page.wait_for_timeout(350)
