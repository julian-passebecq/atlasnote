...
with sync_playwright() as pw:
    ...
    def picker():
        reset('page.atlas.layouts','book');page.get_by_role('button',name='Hide English').click();page.get_by_role('button',name='New tab in pane 1').click()
    ...
    def themes():
        ...
        for theme,label in [('fluent','Microsoft Fluent'),('neutral','Light Minimal'),('academic','Medium / Paper')]:
            page.get_by_role('button',name='Workspace settings').click();page.get_by_label('Light theme').select_option(theme);page.get_by_role('button',name='Close dialog').click();page.wait_for_timeout(300)
        ...
        assert len({c[0] for c in colors})==3
        return {'computedThemeSurfaces':colors,'reload':'Not covered by DOM harness'}
    check('Three light themes visibly differ across semantic surfaces without layout changes',[62,63,64,65,66,68],themes)
    def token_contrast():
        reset(mixed=True);ratios=[]
        for theme in ['fluent','neutral','academic']:
    ...
    def intake():
        ...
        count=page.evaluate('testStore.state.overlays.documents.length');page.get_by_role('button',name='Workspace settings').click();page.get_by_label('Import local PDF').set_input_files({'name':'different-name.pdf','mimeType':'application/pdf','buffer':source});page.get_by_role('button',name='Open existing PDF').click();assert page.evaluate('testStore.state.overlays.documents.length')==count
    ...
    def export_ui():
        reset(mixed=True);page.get_by_role('button',name='Workspace settings').click()
