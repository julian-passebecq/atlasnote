...
with sync_playwright() as pw:
    ...
    def move_archive():
        ...
        page.get_by_role('button',name='PDF content',exact=True).click();page.get_by_label('Selected library resource',exact=True).select_option(before[1]['pageId']);page.get_by_role('button',name='Archive resource',exact=True).click();assert before[1]['pageId'] in page.evaluate('testStore.state.overlays.archived')
        page.get_by_label('Include archived',exact=True).check();page.get_by_label('Selected library resource',exact=True).select_option(before[1]['pageId']);page.get_by_role('button',name='Restore resource',exact=True).click()
    ...
    def intake():
        ...
        show_reader_controls(page);page.get_by_role('button',name='Document info',exact=True).click();assert page.locator('.pdf-info-overlay h3').inner_text()=='Renamed local PDF';page.get_by_role('button',name='Close document info',exact=True).click()
