...
export function mount(built:any){
 ...
 (window as any).testReset=(id='page.atlas.pdf')=>{
  ...
  ws.personal.session.panes[0].views=[v];ws.personal.session.panes[0].active=v.id;ws.personal.session.screen='reader';
  ws.personal.session.libraryMode=c.documents.some(d=>d.pageId===id)?'pdfs':'notes';
  ws.personal.session.expanded=c.projects.flatMap(p=>{const ids=[p.id];const walk=(ns:any[])=>ns.forEach(n=>{if(n.children){ids.push(n.id);walk(n.children);}});walk(p.nodes);return ids;});
  store.setLoaded(ws);
 };
