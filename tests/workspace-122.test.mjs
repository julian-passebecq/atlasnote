...
test('1.2.2 all representative sample notebooks place canonical PDFs before reading',()=>{const samples=source.packs.find(p=>p.manifest.id==='study.samples');assert.equal(samples.projects.length,9);assert.equal(samples.pages.filter(p=>p.id.startsWith('page.samples.')).length,10);assert.equal(samples.pages.filter(p=>p.id.startsWith('page.mock.')).length,11);assert.equal(samples.pages.length,21);for(const p of samples.projects){assert.equal(p.nodes[0].title,'PDFs');assert.equal(p.nodes[1].title,'Start reading');assert.equal(p.nodes[2].title,'Mock playground');}assert.equal(source.packs.find(p=>p.manifest.id==='atlas.reader-guide').projects[0].nodes[0].id,'node.atlas.documents');});
