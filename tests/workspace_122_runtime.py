"""V3 release acceptance: real HTTP origin, integrated PDF.js canvases, actual
...
from browser_support import ROOT,start_server,launch,open_settings,open_context,reader_action,close_panels
...
def goto(p,n,pane=None):
 area=pane or p.locator('.active-pane');field=area.get_by_role('spinbutton',name='Physical PDF page number');field.fill(str(n));field.press('Enter');p.wait_for_timeout(350)
...
try:
 ...
 with sync_playwright() as pw:
  ...
  phase='pdf_engine';workspace(page,2);page.get_by_role('button',name='Informatics filter',exact=True).click();search(page,'PDF reading fixture',True);wait_pdf(page);assert page.locator('.pdf-companion.expanded').count()==1;assert slot(snapshot(page))['libraryMode']=='pdfs';record()
  phase='wheel_single';page.get_by_role('button',name='Collapse PDF Companion',exact=True).click();goto(page,1);wheel(page,150,'bottom');assert number(page)==2;page.wait_for_timeout(600);wheel(page,-150,'top');assert number(page)==1;record()
  ...
  phase='companion_navigation';page.get_by_role('combobox',name='PDF presentation').select_option('single');page.get_by_role('button',name='Expand PDF Companion',exact=True).click();cp=page.locator('.active-pane .pdf-companion');cp.get_by_role('tab',name='Page map',exact=True).click();cp.locator('.companion-page-map button').filter(has_text='Selectable text and language pairs').click();assert number(page)==2
  cp.get_by_role('tab',name='Glossary',exact=True).click();cp.get_by_role('textbox',name='Search this PDF companion',exact=True).fill('Selectable text');cp.locator('.companion-glossary button').click();cp.get_by_role('button',name='Go to PDF page 5',exact=True).click();assert number(page)==5;record()
  phase='companion_edit';cp.get_by_role('button',name='Manage PDF Companion',exact=True).click();page.get_by_label('Edit concept',exact=True).select_option('text-layer');page.get_by_label('Concept translation',exact=True).fill('Valgbar tekst');page.get_by_label('Page occurrences (comma separated)',exact=True).fill('2, 5');page.get_by_label('Category title',exact=True).fill('Verified reading category');page.get_by_role('button',name='Save companion locally',exact=True).click();page.locator('dialog').wait_for(state='detached');saved=snapshot(page);comp=next(iter(saved['overlays']['companions'].values()));assert next(t for t in comp['terms'] if t['id']=='text-layer')['translation']=='Valgbar tekst';record()
  phase='companion_import_validation';cp.get_by_role('button',name='Manage PDF Companion',exact=True).click();bad={**comp,'documentSha256':'0'*64};page.get_by_label('Import companion JSON',exact=True).set_input_files({'name':'bad-companion.json','mimeType':'application/json','buffer':json.dumps(bad).encode()});page.locator('dialog [role=alert]').wait_for();assert snapshot(page)['overlays']['companions']==saved['overlays']['companions']
  ...
  phase='local_ai_input';requests=[];page.on('request',lambda r:requests.append({'method':r.method,'url':r.url}));cp.get_by_role('button',name='Manage PDF Companion',exact=True).click();page.get_by_role('button',name='Prepare AI companion',exact=True).click();page.get_by_role('button',name='Extract locally',exact=True).click();page.get_by_role('button',name='Download AI input part 1',exact=True).wait_for()
  ...
  phase='companion_promotion';cp.get_by_role('tab',name='Glossary',exact=True).click();cp.get_by_role('textbox',name='Search this PDF companion',exact=True).fill('Selectable text');cp.locator('.companion-glossary button').click();cp.get_by_role('button',name='Promote to global glossary',exact=True).click();page.wait_for_timeout(250);promoted=snapshot(page)['overlays']['glossary'];assert len(promoted)==1;assert promoted[0]['pdfRefs'][0]['pages']==[2,5];assert cp.get_by_role('button',name='In global glossary',exact=True).is_disabled();record()
  ...
  phase='compact_controls';page.get_by_role('button',name='Compare in two panes',exact=True).click();wait_pdf(page);cp=page.locator('.active-pane .pdf-companion');cp.get_by_role('button',name='Collapse PDF Companion',exact=True).click();saved_views=slot(snapshot(page))['panes'][0]['views'];height=canvas(page).bounding_box()['height'];page.get_by_role('button',name='Hide reader controls',exact=True).click();page.get_by_role('button',name='Hide global topbar',exact=True).click();page.wait_for_timeout(300);assert canvas(page).bounding_box()['height']>height+65;assert not page.evaluate('!!document.fullscreenElement');assert slot(snapshot(page))['panes'][0]['views']==saved_views
  page.get_by_role('button',name='Show global topbar',exact=True).click();page.get_by_role('button',name='Show reader controls',exact=True).click();record()
  ...
  for w,h,target in [(1366,768,.80),(1440,900,.84),(1920,1080,.87),(390,844,0)]:
   ...
   for name in ['Quick PDF Spread','Hide reader controls','Hide global topbar']:
    r=page.get_by_role('button',name=name,exact=True).bounding_box();assert r and 0<=r['x'] and r['x']+r['width']<=w
   measurements.append({'width':w,'height':h,'companion':'collapsed','canvasFraction':c['height']/pane['height'],'pane':pane,'canvas':c});shot(page,'integrated-'+str(w))
  ...
  page.get_by_role('button',name='Expand PDF Companion',exact=True).click();shot(page,'companion-open-default-study-surface');record(measurements)
  ...
  phase='public_spark_companion';search(page,'Apache Spark: 30 concepts');wait_pdf(page);cp=page.locator('.active-pane .pdf-companion');assert cp.locator('.companion-expanded').is_visible();assert cp.get_by_role('button',name='Collapse PDF Companion',exact=True).is_visible()
  assert page.locator('.active-pane').get_by_label('Physical page count',exact=True).inner_text().strip()=='/ 6'
  goto(page,6);cp.get_by_role('tab',name='Glossary',exact=True).click();cp.get_by_role('textbox',name='Search this PDF companion',exact=True).fill('memory');assert cp.locator('.companion-glossary button').count()>0;shot(page,'actual-public-spark-companion');record({'physicalPages':6,'concepts':30,'reviewed':False,'source':'Pinned byte-verified public PDF, not a substituted renderer'})
