...
try:
 ...
 with sync_playwright() as pw:
  ...
  p.get_by_role('button',name='Switch to PDF library',exact=True).click();assert p.locator('.sidebar-heading').inner_text().strip()=='';assert p.get_by_role('button',name='Switch to notes',exact=True).is_visible();assert p.locator('.tree-project').filter(has_text='Example project').count()==0;shot('pdf-library-mode');p.get_by_role('button',name='Switch to notes',exact=True).click();record(phase)
