...
try:
 ...
 with sync_playwright() as pw:
  ...
  p.get_by_role('button',name='Switch to PDF library',exact=True).click();assert p.locator('.sidebar-heading').inner_text().startswith('PDF Library');assert p.locator('.tree-project').filter(has_text='Example project').count()==0;shot('pdf-library-mode');p.get_by_role('button',name='Switch to notes',exact=True).click();record(phase)
  ...
  phase='focus';p.get_by_role('button',name='Enter focus mode',exact=True).click();p.wait_for_timeout(500);assert p.locator('.focus-mode').count()==1;assert p.locator('.topbar:visible,.pane-tabbar:visible,.reader-rail:visible,.context-drawer:visible').count()==0;assert p.locator('.pdf-controls').is_visible();assert p.locator('.pdf-fallback').count()==0
  ...
  phase='exit';p.keyboard.press('Escape');p.wait_for_timeout(400);assert p.locator('.focus-mode').count()==0;assert not p.evaluate('!!document.fullscreenElement');assert p.locator('.topbar').is_visible();record(phase)
