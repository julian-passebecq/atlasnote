...
try:
    ...
    """
    fixture = json.loads(subprocess.check_output(['node', '--input-type=module', '-e', fixture_code], cwd=ROOT, text=True))
    ...
    with sync_playwright() as pw:
        ...
        expect(page.locator('[data-history-revision-id="' + note['head']['revisionId'] + '"]')).to_have_count(1)
        for width, height in MATRIX:
            ...
            expect(page.locator('[data-revision-id]')).to_have_count(3)
        ...
        expect(page.locator('[data-review-status="accepted"]')).to_be_visible()
        close_panels(page)
