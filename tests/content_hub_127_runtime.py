...
def capture(text,attach=False):
 close_panels(p);click('Quick Capture',p.locator('.sidebar-navigation'));click('Note',p.locator('dialog'));p.get_by_label('Capture text 1',exact=True).fill(text)
...
try:
 ...
 with sync_playwright() as pw:
  ...
  phase='restored_typed_navigation';click('Open Dashboard');p.get_by_role('button',name='Source for Exact question context',exact=True).click();expect(p.locator('.qcm-reader')).to_have_attribute('data-qcm-question','q2');click('Notebook content');folder_toggle=p.locator('[data-project-id="'+folder+'"] > .project-row .tree-expander');
  if folder_toggle.get_attribute('aria-expanded')=='false':folder_toggle.click()
  p.locator('.tree-target').filter(has_text=qcm['title']).first.click();expect(p.locator('.qcm-reader')).to_be_visible();search(article['title']);expect(p.locator('.active-pane')).to_contain_text(article['text']);record()
