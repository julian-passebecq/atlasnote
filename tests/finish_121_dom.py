...
with sync_playwright() as pw:
 ...
 def geometry():
  ...
  for w,h in [(1366,768),(1440,900),(1920,1080),(390,844)]:
   ...
   frame=p.locator('.pdf-fallback');frame.wait_for();pane=p.locator('.document-pane').bounding_box();box=frame.bounding_box();strip=p.locator('.pdf-fallback-strip').bounding_box()
  ...
  return rows
 check('Actual fallback geometry at all four viewports: no intro, compact strip, canvas >=75%',geometry)
 def metadata():
