...
with sync_playwright() as pw:
 ...
 def chrome():
  ...
  nav=p.locator('.sidebar-navigation button').evaluate_all('(es)=>es.map(e=>e.getAttribute("aria-label"))');assert nav==['Switch to PDF library','Collapse notebook sidebar','Back in active tab','Forward in active tab','Global search'],nav
  ...
  classes=p.locator('.pane-tabbar').evaluate('(e)=>[...e.children].slice(0,4).map(x=>x.className)');assert 'pane-identity' in classes[0] and 'pane-new-tab' in classes[1] and 'pane-chrome-toggle' in classes[2] and classes[3]=='tab-list',classes
  ...
  assert p.locator('.state-save-tools button').count()==5
 ...
 def compare():
  ...
  for pane in p.locator('.document-pane').all():
   names=pane.locator('.pane-tabbar').evaluate('(e)=>[...e.children].slice(0,4).map(x=>x.className)');assert 'pane-new-tab' in names[1] and 'pane-chrome-toggle' in names[2]
 ...
 def tree():
  ...
  button('Expand PDF category Reading a PDF').click();assert button('Expand PDF category Layouts and navigation').is_visible();assert not button('Expand PDF category Layouts and navigation').get_attribute('aria-expanded')=='true'
  ...
  button('Expand PDF glossary').click();button('Definition of Physical page').click();assert p.locator('dialog[open]').is_visible();assert 'PDF' in p.locator('dialog').inner_text();p.keyboard.press('Escape')
 ...
 check('PDF title opens without expanding; categories, physical page links and glossary are in sidebar',tree)
 def workspace_save():
  ...
  button('Manage saved states').click();button('Undo last restore').click();p.wait_for_timeout(300);assert session()!=expected
 ...
 def manager():
  reset();button('Save current workspace state').click();p.wait_for_timeout(150);button('Manage saved states').click();button('Rename / note').click();p.get_by_role('textbox',name='Save title',exact=True).fill('Spark - after chapter 2');p.get_by_role('textbox',name='Progress / next step',exact=True).fill('Next: deployment modes.\nKeep going tomorrow.');button('Save details').click();p.wait_for_timeout(200)
 ...
 def allsave():
  ...
  button('Manage saved states').click();p.get_by_role('tab',name='All workspaces saves',exact=True).click();button('Undo last restore').click();p.wait_for_timeout(300);assert state()['activeWorkspaceSlot']==2;assert session()==other
 check('All-workspace last restore and undo restore the active slot without removing manual saves',allsave)
 def isolated():
  reset();button('Save current workspace state').click();p.wait_for_timeout(150);one=session();button('Workspace 2').click();p.wait_for_timeout(150);search('Read in Norwegian, keep English nearby');two=session();button('Manage saved states').click();p.get_by_role('tab',name='Workspace 1 saves',exact=True).click();button('Restore').click();p.wait_for_timeout(250);assert state()['activeWorkspaceSlot']==1;assert session()==one;assert state()['workspaceSlots']['2']==two
 ...
 def responsive():
  ...
  for w,h in [(1366,768),(1440,900),(1920,1080),(390,844)]:
   ...
   for name in ['Enter focus mode','Compare in two panes','Manage saved states','Save current workspace state','Restore last workspace save','Save all workspace states','Restore last all-workspaces save']:
