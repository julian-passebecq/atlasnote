...
const out=path.resolve('docs/evidence/1.2.2/release-gates');fs.mkdirSync(out,{recursive:true});
const commands=[
 ...
 ['workspaces-ui',['run','test:workspaces:ui']], ['integrated-typecheck',['run','typecheck:online']],
...
for(const [id,args] of commands){
 ...
 const row={id,command:'npm '+args.join(' '),exitCode:r.status,signal:r.signal,status:r.status===0?'PASS':'FAIL',durationMs:Date.now()-start,log:id+'.log'};
