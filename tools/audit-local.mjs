...
for(const [folder,entry]of Object.entries(lock.packages)){if(!folder)continue;let installed;try{installed=JSON.parse(await fs.readFile(folder+'/package.json','utf8'));}catch{}
 packages.push({path:folder,name:installed?.name??folder.split('node_modules/').at(-1),version:entry.version,installedVersion:installed?.version??null,optional:entry.optional===true,license:installed?.license??entry.license??'UNRESOLVED',integrity:entry.integrity??null});}
...
const report={scope:'Local installed lockfile packages and unchanged vendor bytes. Does NOT certify current vulnerabilities or complete precompiled Mermaid transitive licenses.',
 ...
 installedVersionsMatch:packages.every(p=>p.installedVersion===null?p.optional:p.version===p.installedVersion),
