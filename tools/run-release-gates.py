...
p.add_argument('--out', type=Path, default=ROOT/'docs/evidence/release-1.2')
...
GATES = [
 ...
 ('reader-ui', [npm,'run','test:reader:ui']), ('compact-ui', [npm,'run','test:compact:ui']),
 ('startup-dom', [npm,'run','test:startup:dom']), ('pdf-authoring', [npm,'run','test:pdf:authoring']),
...
for name,command in GATES:
    ...
    env = {**os.environ, 'ATLAS_EVIDENCE':str(OUT/name),'ATLAS_DIAGNOSTIC_OUT':str(OUT/name),'ATLAS_ASSERT_STARTUP':'1'}
    ...
    data={'release':'1.2.0','timestamp':datetime.now(timezone.utc).isoformat(),'scope':'Local offline gates; no remote registry audit, push, CI or deployment. Each browser result states DOM or real-origin scope.','gates':records}
