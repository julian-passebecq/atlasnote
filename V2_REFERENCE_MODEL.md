...
Status: implemented, unreleased. See FINAL_TEST_STATUS.md for the verification boundary. The stabilized delivery continues the audited V2 source built on 1.2.7; no application restart or new storage service was introduced.
...
## Stabilization source-edit and demo boundaries

The canonical structures above are unchanged. The new validated resource drag envelope carries the existing ReadingTarget and no persisted duplicate content. Source replacement operates on cloned overlays, checks the currently effective source identity for staleness, validates new targets, and rejects changes that would degrade an existing exact reference. A PDF-category source-edit check preserves the category ID separately because semantic matching intentionally canonicalizes it to its physical page. Existing QCM attempt question/option identities remain protected.

Optional demo operations use the existing personal and overlay stores in one validated IndexedDB transaction. No new database, version, object store, backup envelope or concept schema is introduced. Demo IDs are deterministic and namespace-scoped. Cleanup is conservative: user edits/references/attempts/current and saved reader state protect records and dependencies. The full backup continues to carry canonical personal/overlay data through the existing envelope.

The new manager's staged JSON and parent-owned capture/Article drafts are transient UI workflow state, not a second content model. A cancelled draft does not become a Dashboard record. See the stabilization test report for the real-origin persistence verification limit.
