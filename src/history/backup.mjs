...
export async function writeHistoryFiles(files,history){
 ...
 files.set('history/index.json',JSON.stringify({schemaVersion:1,meta:history.meta,heads:history.heads,revisionShards,reviewShards,hash:await sha256(stable(history))}));return {schemaVersion:1,indexPath:'history/index.json'};
}
export async function readHistoryFiles(files,root,pointer,assets){
 ...
 const index=read(pointer.indexPath);strictKeys(index,['schemaVersion','meta','heads','revisionShards','reviewShards','hash'],'history index');if(index.schemaVersion!==1||!Array.isArray(index.revisionShards)||!Array.isArray(index.reviewShards)||index.revisionShards.length+index.reviewShards.length>1000)throw Error('Invalid history shards');
 ...
 const h={schemaVersion:1,meta:index.meta,heads:index.heads,revisions:index.revisionShards.flatMap(read),reviews:index.reviewShards.flatMap(read)};
