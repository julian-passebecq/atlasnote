...
export function resourceKeyForTarget(c:Catalogue,t:ResourceTarget):string|undefined {
 ...
 if(t.kind==='article'){const p=c.pages.find(p=>p.article?.id===t.articleId&&(!t.pageId||p.id===t.pageId));return p?'article:'+p.id:undefined;}
 if(t.kind==='qcm'){const p=c.pages.find(p=>p.qcm?.id===t.setId&&(!t.pageId||p.id===t.pageId));return p?'qcm:'+p.id:undefined;}
 if(t.kind==='pdf-page'||t.kind==='pdf-category'){const d=c.documents.find(d=>d.pageId===t.pageId&&d.id===t.documentId);return d?'pdf:'+d.id:undefined;}
 if(t.kind==='cheatsheet-page'){const p=c.pages.find(p=>p.id===t.pageId&&p.cheatsheet?.id===t.documentId);return p?'cheatsheet:'+p.id:undefined;}
 const d=c.documents.find(d=>d.pageId===t.pageId),p=c.pages.find(p=>p.id===t.pageId);return d?'pdf:'+d.id:p?pageType(p)+':'+p.id:undefined;
}
