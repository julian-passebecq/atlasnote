...
export class WorkspaceStore{
 ...
 async restore(ws:Workspace){await this.flush();return this.enqueue(async()=>{await preflightStorage(workspaceImportBytes(ws.imports,ws.assets,ws.overlays)+new TextEncoder().encode(JSON.stringify(ws.history??{})).length);const restored=await restoreWorkspace(ws);clearArchiveAttachments();this.state={...restored,generation:this.state.generation+1};this.error='';this.emit();},false);}
