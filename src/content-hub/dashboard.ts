...
/** First line is the display title; the remainder is shown as a short preview. */
export function splitCaptureText(text:string):{title:string;rest:string}{const lines=text.split(/\r?\n/),first=lines.findIndex(l=>l.trim());if(first<0)return {title:text,rest:''};return {title:lines[first].trim(),rest:lines.slice(first+1).join('\n').trim()};}
