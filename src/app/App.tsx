...
import {historicalCatalogue,historicalWorkspace,resourceKeyForTarget} from '../history/adapters.js';
...
function StudyWorkspace({ built,toast,setToast }: any) {
    ...
    function showHistory(id?:string){const c=catalogueRef.current,key=id?.includes(':')?id:id?resourceKeyForTarget(c,targetForPage(c,id)):activeResourceKey;if(key){setPopover(null);setModal({kind:'history',resourceKey:key});}else notify('Open a versioned resource first.',true);}
    async function previousVersion(id:string,compare=true){try{const c=catalogueRef.current,key=id.includes(':')?id:resourceKeyForTarget(c,targetForPage(c,id));if(!key)throw Error('Resource history is unavailable.');const api=getAgentInterface(),rows=api.listResourceVersions(key).items;if(rows.length<2)throw Error('This resource has no previous version yet.');if(compare)await api.compareRevisions(key,rows[1].revisionId);else await api.navigateAgentTarget(api.getResource(key,rows[1].revisionId).target,'pane');}catch(e){notify((e as Error).message,true);}}
    useEffect(()=>connectAgentUI({'history':target=>{const c=catalogueRef.current;showHistory(target?resourceKeyForTarget(c,target):undefined);},'agent-review':()=>setModal({kind:'agent-review'}),'capture':()=>capture(),'states':()=>setModal({kind:'saved-states'})}),[activeResourceKey]);
...
function toggleChrome(paneId:string){document.dispatchEvent(new CustomEvent('atlas:reader-chrome',{detail:{paneId}}));setSession(s=>{const p=s.panes.find(p=>p.id===paneId);if(p)p.readerChromeCollapsed=!(p.readerChromeCollapsed??true);});}
 ...
 setSession(s => { revealPane(s,paneId);const pane=s.panes.find(p=>p.id===paneId);followDocument(s,current(pane?.views.find(v=>v.id===pane.active))?.pageId); }); setMobilePane(paneId); }
    ...
    useEffect(() => {
        const onHash=(initial=false)=>{try{const route=parseHashRoute(location.hash);if(route&&(!initial||shouldOpenStartupRoute(activeSession(store.state.personal),route))){if(route.historyRevisionId)openTarget({...targetForPage(catalogueRef.current,route.id,route.anchor),historyRevisionId:route.historyRevisionId});else openPage(route.id,route.anchor);}}catch{notify('This page link is malformed.',true);}};
 ...
 return <section key={pane.id} data-pane-id={pane.id} data-pane-slot={index===0?'a':'b'} data-history-revision-id={loc?.historyRevisionId??'current'} data-current-revision-id={page?ws.history?.heads.find(h=>h.resourceKey===resourceKeyForTarget(paneCatalogue,targetForPage(paneCatalogue,page.id)))?.revisionId:undefined} data-resource-key={page?resourceKeyForTarget(paneCatalogue,targetForPage(paneCatalogue,page.id)):undefined} className={'document-pane '+(active?'active-pane ':'')+((pane.readerChromeCollapsed??true)?'reader-chrome-hidden ':'')+(session.panes.length===2&&pane.id!==mobilePane?'mobile-inactive':'')} style={paired?{flex:'0 0 '+(index===0?'calc(var(--ratio,50%) - 4px)':'calc(100% - var(--ratio,50%) - 4px)')}:{flex:'1'}} onPointerDownCapture={()=>activate(pane.id)} onFocusCapture={()=>activate(pane.id)} aria-label={session.panes.length===2?'Reading pane '+(index+1):'Reading pane'}>
 ...
 {loc?.historyRevisionId&&<div className="historical-banner" role="status"><strong>{projection.revision?'Version '+projection.revision.number+' / Read-only snapshot':'Historical version unavailable'}</strong><code>{loc.historyRevisionId}</code>{projection.revision&&<button onClick={()=>showHistory(projection.revision!.resourceKey)}>Version History</button>}<button onClick={()=>{const target=currentResourceTarget(catalogue,loc);if(target){const {historyRevisionId,...floating}=target;openTarget(floating);}}}>Open current</button></div>}
 {projection.warning?<div className="missing-page" role="alert"><h2>Historical content unavailable</h2><p>{projection.warning}</p><button onClick={settings}>Restore full backup</button></div>:view?.referenceExplorer?<ReferenceExplorer {...referenceProps} state={view.referenceExplorer} target={view.referenceExplorer.target} onState={value=>updateView(pane.id,view.id,v=>{v.referenceExplorer=value;})} onClose={()=>closeTab(pane.id,view.id)}/>:view&&loc?.collectionId&&projection.revision?<div className="history-readonly-summary"><h2>{projection.revision.snapshot.project?.title}</h2><p>Historical Notebook structure. Source references remain references; restoring this version creates a new structure revision.</p><pre>{JSON.stringify(projection.revision.snapshot.project?.nodes,null,2)}</pre></div>:view&&loc?.collectionId?<CollectionView onEditReference={item=>setModal({kind:'reference-edit',reference:item})} onResource={openTarget} onReferenceActions={showReadingActions} key={view.id+loc.collectionId} id={loc.collectionId} catalogue={catalogue} workspace={ws} onOpen={(id:string,anchor?:Anchor,newTab=false)=>openPage(id,anchor,newTab,pane.id)} onOther={session.panes.length===2?(id:string)=>{const other=session.panes.find(p=>p.id!==pane.id);if(other)openPage(id,undefined,false,other.id);}:undefined} onManage={(item:any)=>setModal({kind:'manage',item})} onBookmark={bookmarkDocument} onCreate={setModalCreate} onImport={(projectId:string,folderId?:string)=>setModal({kind:'settings',projectId,folderId})}/>:
 ...
 {modal?.kind==='history'&&<HistoryPanel built={built} workspace={ws} resourceKey={modal.resourceKey} onClose={()=>setModal(null)} notify={notify}/>}
