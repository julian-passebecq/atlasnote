...
export function App({ built }: any) {
    ...
    function toggleFocus(){setSession(s=>{s.focus=!s.focus;});}
    ...
    useEffect(() => { const onHash = () => { const m = location.hash.match(/^#\/(?:page|collection)\/([^?]+)(?:\?(.*))?$/); if (m) {
        const p = new URLSearchParams(m[2] ?? '');
        try {
            openPage(decodeURIComponent(m[1]), p.get('block') ? { blockId: p.get('block')! } : undefined);
        }
        catch {
            notify('This page link is malformed.', true);
        }
    ...
    const leftVisible = session.leftOpen && !session.focus && (!small || mobileSide === 'left'), rightVisible = session.rightOpen && !session.focus && (!small || mobileSide === 'right');
 ...
 <header className="topbar"><button className="brand" onClick={() => setSession(s => { s.screen = 'home'; })} aria-label="Knowledge Atlas home"><img src={new URL('assets/atlas.svg', document.baseURI).href} alt=""/><strong>Knowledge Atlas</strong><span className="beta-label">LOCAL</span></button><div className="topbar-history"><IconButton name="left" label="Back in active tab" disabled={!activeView || activeView.cursor <= 0} onClick={() => historyStep(-1)}/><IconButton name="right" label="Forward in active tab" disabled={!activeView || activeView.cursor >= activeView.history.length - 1} onClick={() => historyStep(1)}/></div><button className="global-search" onClick={() => setModal({ kind: 'search' })}><Icon name="search" size={17}/><span>Search your knowledge</span><kbd>Ctrl K</kbd></button><div className="topbar-actions"><button className="export-button" onClick={() => activePage ? setModal({ kind: 'export' }) : notify('Open a page before exporting.')} disabled={!activePage}><Icon name="export" size={17}/><span>Export to AI</span></button><IconButton name="print" label="Print or Save as PDF" disabled={!activePage} onClick={() => setModal({ kind: 'print' })}/><IconButton name="focus" label={session.focus ? 'Exit focus mode' : 'Enter focus mode'} active={session.focus} onClick={toggleFocus}/><button className={'compare-button ' + (session.panes.length === 2 ? 'selected' : '')} title={session.panes.length === 2 ? 'Close Compare and keep the active pane' : 'Compare in two panes'} aria-label="Compare in two panes" aria-pressed={session.panes.length === 2} disabled={session.panes.length !== 2 && !activeLocation} onClick={compare}><Icon name="split"/><span>Compare</span></button>{session.panes.length === 2 && <IconButton name="swap" label="Swap panes" onClick={() => setSession(s => { s.panes.reverse(); s.ratio = 100 - s.ratio; })}/>}<IconButton name="flag" label="Show learning flags" active={session.showFlags} onClick={() => setSession(s => { s.showFlags = !s.showFlags; })}/></div></header>
 ...
 <div className="workspace-frame"><nav className="utility-rail" aria-label="Workspace tools"><IconButton name="panel" label={leftVisible ? 'Collapse notebook sidebar' : 'Open notebook sidebar'} active={leftVisible} onClick={() => setSession(s => { s.leftOpen = small ? true : !leftVisible; if (small)
        setMobileSide(mobileSide === 'left' ? '' : 'left'); if (s.focus)
        s.focus = false; })}/><IconButton name="home" label="Home" active={session.screen === 'home'} onClick={() => setSession(s => { s.screen = 'home'; })}/><IconButton name="search" label="Global search" onClick={() => setModal({ kind: 'search' })}/><IconButton name="bookmark" label="Bookmarks" active={session.screen === 'bookmarks'} onClick={() => setSession(s => { s.screen = 'bookmarks'; })}/><span className="rail-spacer"/><IconButton name="settings" label="Workspace settings" onClick={settings}/></nav>
 {leftVisible && <ProjectTree catalogue={catalogue} workspace={ws} activePage={session.screen === 'reader' ? activeLocation?.pageId : undefined} onCollection={openPage} onOpen={openPage} onOther={session.panes.length === 2 ? openOther : undefined} onBookmark={bookmarkDocument} onToggle={(id: string) => setSession(s => { s.expanded = s.expanded.includes(id) ? s.expanded.filter(x => x !== id) : [...s.expanded, id]; })} onItem={(item: any) => setModal({ kind: 'manage', item })} onCreate={setModalCreate}/>}
 ...
 </div>{rightVisible ? <ContextPanel catalogue={catalogue} page={session.screen === 'reader' ? activePage : undefined} view={activeView} location={activeLocation} workspace={ws} onOpen={openPage} onJump={jump} onClose={() => setSession(s => { if (small)
        setMobileSide('');
    else
        s.rightOpen = false; })} notify={notify}/> : <aside className="context-rail"><IconButton name="context" label="Open context panel" onClick={() => setSession(s => { s.rightOpen = true; if (small)
        setMobileSide('right'); if (s.focus)
        s.focus = false; })}/></aside>}
