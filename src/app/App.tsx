...
import {getAgentInterface,connectAgentUI} from '../agent/service.js';
import {historicalCatalogue,historicalWorkspace,resourceKeyForTarget,REFERENCES_PROJECT} from '../history/adapters.js';
import {createHistoryIndex,historyContextForTarget,historyIdentityAttributes} from '../history/ui-context.js';
import {prepareDemo} from '../stabilization/demo.js';
import {newCaptureDraft,resumeArticleDraft} from '../stabilization/capture-draft.js';
import {ReadingTargetDialog} from '../stabilization/ReadingTargetDialog.js';
import {toggleDashboard} from '../content-hub/surfaces.js';
...
import {ExperiencePanel} from '../experience/ExperiencePanel.js';
import {NorskDailyView} from '../norsk-daily/NorskDailyView.js';
...
function StudyWorkspace({ built,toast,setToast }: any) {
    ...
    const [experienceSlot,setExperienceSlot]=useState<import('../core/model.js').WorkspaceNumber|null>(null);
    ...
    const activePane = session.panes.find(p => p.id === session.activePane) ?? session.panes[0], activeView = activePane?.views.find(v => v.id === activePane.active), activeLocation = current(activeView), activeProjection=historicalCatalogue(catalogue,ws,activeLocation?.historyRevisionId), activePage = activeProjection.warning?undefined:activeProjection.catalogue.pages.find(p => p.id === activeLocation?.pageId);
    ...
    function showHistory(id?:string){
      ...
      else notify('History is unavailable for this resource. Complete initialization or import its full backup.',true);
    }
    async function previousVersion(key:string,compare=true,selectedRevisionId?:string){
      try{
        ...
        if(compare)await api.compareRevisions(key,revision.parentRevisionId,selectedRevisionId);
        else await api.navigateAgentTarget(previous.target,'pane');
        setReadingMenu(null);setMobileSide('');syncHash(activeSession(store.state.personal));
      }catch(e){notify((e as Error).message,true);}
    }
    useEffect(()=>connectAgentUI({beforeNavigation:captureReaders,afterNavigation:()=>{syncHash(activeSession(store.state.personal));setPopover(null);setReadingMenu(null);setContextOpen(false);setMobileSide('');},'history':target=>{if(!target){showHistory();return;}const context=historyContextForTarget(catalogueRef.current,store.state,target);if(!context||context.unavailableReason)throw Error(context?.unavailableReason??'This target has no content history.');showHistory(context.resourceKey);},'agent-review':()=>setModal({kind:'agent-review'}),'capture':()=>capture(),'states':()=>setModal({kind:'saved-states'})}),[activeResourceKey]);
    ...
    function setSession(fn: (s: Session) => void, checkpoint=false) {
 ...
 // V3: a reader scroll checkpoint only moves a position inside the current
 // resource. It is coalesced durably and does not re-capture every reader.
 if(checkpoint){store.checkpoint(p => fn(activeSession(p,slotId)));return;}
 captureReaders();store.personal(p => fn(activeSession(p,slotId)));
 syncHash(activeSession(store.state.personal,slotId));
}
function syncHash(s:Session){const pane=s.panes.find(p=>p.id===s.activePane),loc=current(pane?.views.find(v=>v.id===pane.active));history.replaceState(null,'',s.screen==='reader'&&loc?(loc.collectionId?'#/collection/':'#/page/')+encodeURIComponent(loc.pageId)+(loc.historyRevisionId?'?revision='+encodeURIComponent(loc.historyRevisionId):''):'#/');}
...
function toggleChrome(paneId:string){document.dispatchEvent(new CustomEvent('atlas:reader-chrome',{detail:{paneId}}));setSession(s=>{const p=s.panes.find(p=>p.id===paneId);if(p)p.readerChromeCollapsed=!(p.readerChromeCollapsed??true);});}
    ...
    function demoData(action:'load'|'remove'){if(action==='remove'&&!confirm('Remove unchanged demonstration data? Edited items and sources referenced by your work are kept.'))return;try{captureReaders();const prepared=prepareDemo(catalogueRef.current,store.state,action);void store.shared((p,o)=>{Object.assign(p,prepared.personal);Object.assign(o,prepared.overlays);}).then(()=>notify(prepared.message)).catch(e=>notify(e.message,true));}catch(e){notify((e as Error).message,true);}}
    function showDashboard(){setPopover(null);setContextOpen(false);setSession(toggleDashboard);}
    function capture(mode:any='link'){captureReaders();const s=activeSession(store.state.personal),p=s.panes.find(p=>p.id===s.activePane),l=current(p?.views.find(v=>v.id===p.active));setPopover(null);setModal({kind:'capture',draft:newCaptureDraft(['link','task','note'].includes(mode)?mode:'link'),target:currentResourceTarget(historicalCatalogue(catalogueRef.current,store.state,l?.historyRevisionId).catalogue,l),suggestion:l?resourceTaxonomy(catalogueRef.current,store.state.overlays,l.pageId):undefined});}
    const [managedResources,setManagedResources]=useState<Record<string,string>>({});
    function manageResource(id:string){if(small)setMobileSide('');const mode=libraryModeForPage(catalogueRef.current,id);setManagedResources(m=>({...m,[mode]:id}));setSession(s=>{selectLibrary(s,mode);});}
    function addReading(kind:'bookmark'|'later'){captureReaders();setModal({kind:'reading-target',readingKind:kind,target:currentResourceTarget(historicalCatalogue(catalogueRef.current,store.state,current(activeView)?.historyRevisionId).catalogue,current(activeView))});}
    ...
    function navigation(){return <nav className="sidebar-navigation" aria-label="Library navigation">
      ...
      <span className="navigation-separator" aria-hidden="true"/><IconButton name="plus" label="Quick Capture" onClick={()=>capture()}/><IconButton name="grid" label="Open Dashboard" active={session.surface==='dashboard'} onClick={showDashboard}/>
    ...
    function addCurrent(kind:'bookmark'|'later'){
      ...
      const target=currentResourceTarget(historicalCatalogue(catalogueRef.current,store.state,l?.historyRevisionId).catalogue,l)??targetForPage(catalogueRef.current,l.pageId,l.anchor),title=catalogueRef.current.pages.find(p=>p.id===l.pageId)?.title??collectionTarget(catalogueRef.current,l.pageId)?.title??'Saved reading';
    ...
    function managerPanel(){
      ...
      return <ContextPanel references={referenceProps} resolveAsset={assets.url} catalogue={activeProjection.catalogue} page={session.screen==='reader'?activePage:undefined} view={activeView} location={activeLocation} workspace={historicalWorkspace(ws,activeLocation?.historyRevisionId)} paneId={activePane?.id} onTarget={openTarget} onOpen={id=>openTarget(targetForPage(catalogue,id))} onJump={jump} onPdfNavigate={(id:string,n:number)=>{if(activeLocation?.historyRevisionId){const doc=activeProjection.catalogue.documents.find(d=>d.pageId===id);if(doc)openTarget({kind:'pdf-page',pageId:id,documentId:doc.id,pdfPage:n,revision:doc.sha256,historyRevisionId:activeLocation.historyRevisionId});}else pdfNavigate(id,n);}} onPdfToggle={togglePdfTree} onPdfManage={(doc:any)=>{if(activeLocation?.historyRevisionId)showHistory('pdf:'+doc.id);else setModal({kind:'companion',doc});}} onReadingActions={showReadingActions} onClose={()=>setContextOpen(false)} notify={notify}/>;
    }
    ...
    function pdfNavigate(id:string,n:number,newTab=false){
      ...
      if(existing&&!newTab){setSession(s=>{const p=s.panes.find(p=>p.id===pane.id)!;p.active=existing.id;const l=current(p.views.find(v=>v.id===existing.id))!;delete l.historyRevisionId;l.pdfPage=n;l.anchor={pdfPage:n,...(doc.sha256?{pdfRevision:doc.sha256}:{})};s.screen='reader';revealPane(s,p.id);followDocument(s,id);});setMobileSide('');setModal(null);}
    ...
    function updateView(paneId: string, viewId: string, fn: (v: View) => void, checkpoint=false) { setSession(s => { const v = s.panes.find(p => p.id === paneId)?.views.find(v => v.id === viewId); if (v)
        fn(v); }, checkpoint); }
 ...
 setSession(s => { revealPane(s,paneId);const pane=s.panes.find(p=>p.id===paneId);followDocument(s,current(pane?.views.find(v=>v.id===pane.active))?.pageId); }); setMobilePane(paneId); }
    ...
    function jump(blockId: string, paneId = activePane?.id, viewId = activeView?.id) { if (!paneId || !viewId)
        return; updateView(paneId, viewId, v => { const loc = current(v)!; const page = historicalCatalogue(catalogueRef.current,store.state,loc.historyRevisionId).catalogue.pages.find(p => p.id === loc.pageId); if(page?.cheatsheet){const anchor=cheatsheetAnchor(page.cheatsheet,blockId);if(anchor){loc.anchor=anchor;loc.sheetPage=anchor.sheetPage;loc.scroll=0;}return;}if (page)
    ...
    function bookmark(page: Page, view: View) { const loc = current(view)!, anchor = loc.anchor; store.personal(p => { const found = p.bookmarks.find(b => b.pageId === page.id && b.target?.historyRevisionId === loc.historyRevisionId && JSON.stringify(b.anchor ?? {}) === JSON.stringify(anchor ?? {})); if (found)
        p.bookmarks = p.bookmarks.filter(b => b.id !== found.id);
    else
    ...
    useEffect(() => {
        const onHash=(initial=false)=>{try{
          ...
          if(route.historyRevisionId){
            ...
            if(!route.collection&&!projected.catalogue.pages.some(p=>p.id===route.id))throw Error('This historical page is unavailable; no current resource was substituted.');
            const target:ReadingTarget=route.collection?{kind:'collection',collectionId:route.id}:targetForPage(projected.catalogue,route.id,route.anchor);
            openTarget({...target,historyRevisionId:route.historyRevisionId});
          }else if(route.collection)openTarget({kind:'collection',collectionId:route.id});
          else openPage(route.id,route.anchor);
        }catch(e){notify(e instanceof Error?e.message:'This page link is malformed.',true);}};
 ...
 const letter=index===0?'A':'B',view=pane.views.find(v=>v.id===pane.active),loc=current(view),projection=historicalCatalogue(catalogue,ws,loc?.historyRevisionId),paneCatalogue=projection.catalogue,page=projection.warning?undefined:paneCatalogue.pages.find(p=>p.id===loc?.pageId),doc=paneCatalogue.documents.find(d=>d.pageId===page?.id),sheet=page?.cheatsheet,crumb=page?locs.get(page.id):undefined,active=pane.id===session.activePane;
 if(session.collapsedPane===pane.id)return <aside className="collapsed-pane" data-pane-id={pane.id} data-pane-slot={index===0?'a':'b'} key={pane.id}><button aria-label={'Restore pane '+letter} onClick={()=>setSession(s=>{revealPane(s,pane.id);followDocument(s,loc?.pageId);})}>{index===0?<>{letter}<Icon name="right" size={16}/></>:<><Icon name="left" size={16}/>{letter}</>}</button></aside>;
 const paired=session.panes.length===2&&!session.collapsedPane;
 const paneHistory=historyContextForTarget(catalogue,ws,currentResourceTarget(paneCatalogue,loc),historyIndex);
 return <section key={pane.id} data-pane-id={pane.id} data-pane-slot={index===0?'a':'b'} {...historyIdentityAttributes(paneHistory)} data-history-revision-id={loc?.historyRevisionId??'current'} data-workspace-id={slotId} className={'document-pane '+(active?'active-pane ':'')+((pane.readerChromeCollapsed??true)?'reader-chrome-hidden ':'')+(session.panes.length===2&&pane.id!==mobilePane?'mobile-inactive':'')} style={paired?{flex:'0 0 '+(index===0?'calc(var(--ratio,50%) - 4px)':'calc(100% - var(--ratio,50%) - 4px)')}:{flex:'1'}} onPointerDownCapture={()=>activate(pane.id)} onFocusCapture={()=>activate(pane.id)} aria-label={session.panes.length===2?'Reading pane '+(index+1):'Reading pane'}>
 <div className="pane-tabbar">
 {session.panes.length===2&&<button className={'pane-identity '+(active?'is-active':'')} aria-label={'Collapse pane '+letter} title={'Collapse pane '+letter+' without closing its tabs'} disabled={!!session.collapsedPane} onClick={()=>setSession(s=>{collapsePane(s,pane.id);const target=s.panes.find(p=>p.id===s.activePane);followDocument(s,current(target?.views.find(v=>v.id===target.active))?.pageId);})}>{index===0?<>{letter}<Icon name="left" size={14}/></>:<><Icon name="right" size={14}/>{letter}</>}</button>}
 ...
 const route=current(v),tabProjection=historicalCatalogue(catalogue,ws,route?.historyRevisionId),tabCatalogue=tabProjection.catalogue,p=tabProjection.warning?undefined:tabCatalogue.pages.find(p=>p.id===route?.pageId),collection=route?.collectionId&&!tabProjection.warning?collectionTarget(route.collectionId.startsWith('project.unfiled.')?{...tabCatalogue,projects:projectLibrary(tabCatalogue,ws.overlays,'notes')}:tabCatalogue,route.collectionId):undefined;
 const tabHistory=historyContextForTarget(catalogue,ws,currentResourceTarget(tabCatalogue,route),historyIndex);
 return <div role="tab" data-agent-action="tab-select" data-tab-id={v.id} {...historyIdentityAttributes(tabHistory)} aria-selected={v.id===pane.active} tabIndex={v.id===pane.active?0:-1} className={'document-tab '+(v.id===pane.active?'selected':'')} key={v.id} title={v.referenceExplorer?'References':p?.title??collection?.title??'New tab'} onClick={()=>selectTab(pane.id,v.id)} onKeyDown={e=>{
 ...
 }}><Icon name={p?.cheatsheet?'grid':tabCatalogue.documents.some(d=>d.pageId===p?.id)?'pdf':'page'} size={15}/><span>{route?.historyRevisionId&&<small className="tab-version-label">{tabProjection.revision?'v'+tabProjection.revision.number:'Missing version'} / </small>}{v.referenceExplorer?'References':p?.title??collection?.title??(route?'Unavailable':'New tab')}</span>{p&&outsideExperience(p.id)&&<small className="tab-outside" title="Outside this workspace's Experience. It stays open; nothing was deleted.">Outside</small>}<button className="tab-close" data-agent-action="tab-close" data-tab-id={v.id} aria-label={'Close tab '+(v.referenceExplorer?'References':p?.title??collection?.title??'new tab')} onClick={e=>{e.stopPropagation();closeTab(pane.id,v.id);}}><Icon name="close" size={13}/></button></div>;
 })}</div>
 ...
 {page?page.qcm?<QcmReader readOnly={!!loc.historyRevisionId} key={view.id+page.id+(loc.historyRevisionId??'current')} page={page} location={loc} catalogue={paneCatalogue} workspace={ws} onLocation={patch=>updateView(pane.id,view.id,v=>{const l=current(v);if(l?.pageId===page.id)Object.assign(l,patch);})} onOpen={openTarget} onActions={showReadingActions}/>:sheet?<CheatsheetReader key={view.id+page.id+(loc.historyRevisionId??'current')} page={page} view={view} location={loc} paneId={pane.id} slotId={slotId} onActions={showReadingActions} onLocation={patch=>updateView(pane.id,view.id,v=>{const entry=current(v);if(entry?.pageId===page.id)Object.assign(entry,patch);})}/>:doc&&doc.assetKey&&!store.hydrated&&!assets.url(doc.assetKey)?<div className="pdf-engine-empty" role="status"><h1>{doc.title}</h1><p>Loading your local library…</p></div>:doc?<PdfReader key={view.id+doc.id+(loc.historyRevisionId??'current')} paneId={pane.id} slotId={slotId} document={doc} location={loc} resolve={assets.url} fetchBytes={assets.bytes} onRenderer={(mode:string)=>setPdfRenderers(prior=>prior[view.id+doc.id]===mode?prior:{...prior,[view.id+doc.id]:mode})} onLocation={(patch:any,meta?:{checkpoint?:boolean})=>updateView(pane.id,view.id,v=>{const entry=current(v);if(entry?.pageId===doc.pageId)Object.assign(entry,patch);},!!meta?.checkpoint)}/>:
 <NoteReader key={view.id+page.id+(loc.historyRevisionId??'current')} page={page} view={view} location={loc} resolveAsset={(path:string,p:Page)=>assets.inSnapshot(path,p,projection.revision?.snapshot.assetRefs)} fontSize={session.fontSize} layoutKey={session.theme+'|'+session.focus+'|'+session.panes.length+'|'+session.compactTop+'|'+pane.readerChromeCollapsed+'|'+session.collapsedPane} onAnchor={a=>updateView(pane.id,view.id,v=>{const entry=current(v);if(entry?.pageId===page.id)entry.anchor=a;})} onAction={(action:any,id:any,snippet:any,event:any)=>blockAction(action,id,snippet,pane.id,view,page,event)} onLink={(id:string,anchor?:Anchor,newTab?:boolean)=>openPage(id,anchor,newTab,pane.id)}/>:
 ...
 {!store.error&&(store.staleNotice||store.conflictNotice)&&<div className="storage-banner tab-sync-banner" role="status"><span>{store.staleNotice||store.conflictNotice}</span>{store.staleNotice&&<button onClick={async()=>{await store.flush();location.reload();}}>Reload</button>}<button onClick={()=>store.dismissNotices()}>Dismiss</button></div>}
 ...
 {readingMenu&&<ReadingActions key={readingMenu.target.kind+readingMenu.title} menu={readingMenu} historyContext={historyContextForTarget(catalogue,ws,readingMenu.target,historyIndex)} onHistory={showHistory} onComparePrevious={(key,revision)=>void previousVersion(key,true,revision)} onOpenPrevious={(key,revision)=>void previousVersion(key,false,revision)} onClose={()=>setReadingMenu(null)} onOpen={where=>openTarget(readingMenu.target,where,readingMenu.semanticReference)} onLater={()=>void saveTarget('later',readingMenu.target,readingMenu.title).catch(()=>{})} onBookmark={()=>void saveTarget('bookmark',readingMenu.target,readingMenu.title).catch(()=>{})}/>}
 ...
 {modal?.kind==='agent-review'&&<AgentReviewDialog workspace={ws} resourceKey={activeResourceKey} initialChangeSet={modal.changeSet} initialStatus={modal.status} onClose={()=>setModal(null)}/>}
 ...
 {modal?.kind==='reading-target'&&<ReadingTargetDialog kind={modal.readingKind} catalogue={catalogue} workspace={ws} current={modal.target} onSave={(t,title)=>saveTarget(modal.readingKind,t,title)} onClose={()=>setModal(null)}/>}
 {(modal?.kind==='content-editor'||modal?.kind==='edit'&&(modal.page?.article||modal.page?.qcm))&&<ContentEditor onBack={modal.captureFlow?source=>setModal({...modal,kind:'capture',articleDraft:source}):undefined} kind={modal.contentKind??(modal.page?.qcm?'qcm':'articles')} page={modal.page} catalogue={catalogue} initialSource={modal.source} onClose={()=>setModal(null)} onOpen={openPage} notify={notify}/>}
 ...
 {experienceSlot&&<ExperiencePanel key={experienceSlot} slot={experienceSlot} session={experienceSlot===1?ws.personal.session:ws.personal.workspaceSlots?.[experienceSlot]} catalogue={catalogue} overlays={ws.overlays} facts={experienceFacts} onApply={applyExperience} onClose={()=>setExperienceSlot(null)}/>}
 {modal?.kind === 'search' && <SearchDialog experience={experience} facts={experienceFacts} catalogue={catalogue} onClose={() => setModal(null)} onOpen={(id: string, anchor?: Anchor, newTab = false) => openPage(id, anchor, modal.newTab || newTab, modal.paneId)}/>}
 ...
 {['export', 'print'].includes(modal?.kind) && activePage && <ExportDialog catalogue={activeProjection.catalogue} page={activePage} view={activeView} onClose={() => setModal(null)} notify={notify} print={modal.kind === 'print'} onPrint={(selection: any) => { setModal(null); setPrintSelection(selection); }}/>}
