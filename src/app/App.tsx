...
function toggleChrome(paneId:string){document.dispatchEvent(new CustomEvent('atlas:reader-chrome',{detail:{paneId}}));setSession(s=>{const p=s.panes.find(p=>p.id===paneId);if(p)p.readerChromeCollapsed=!(p.readerChromeCollapsed??true);});}
 ...
 <IconButton className="pane-chrome-toggle" name="chrome" label={(pane.readerChromeCollapsed??true)?'Show reader controls':'Hide reader controls'} active={pane.readerChromeCollapsed??true} onClick={()=>toggleChrome(pane.id)}/>
 ...
 onPdfManage={()=>{const doc=catalogue.documents.find(d=>d.pageId===activePage?.id);if(doc)setModal({kind:'companion',doc});}} onSettings={settings} onHome={()=>setSession(s=>{s.screen='home';})} onBookmarks={()=>setSession(s=>{s.screen='bookmarks';})} onEdit={()=>setModal({kind:'edit',page:activePage})} onPrint={()=>setModal({kind:'print'})}
 ...
 {modal?.kind==='companion'&&<CompanionPanel managerOnly onClose={()=>setModal(null)} document={modal.doc} physicalPage={activeLocation?.pageId===modal.doc.pageId?(activeLocation?.pdfPage??1):1} paneId={activePane?.id} slotId={slotId} onNavigate={n=>pdfNavigate(modal.doc.pageId,n)} prepare={activeLocation?.pageId===modal.doc.pageId&&pdfRenderers[(activeView?.id??'')+modal.doc.id]==='integrated'?prepareOpenPdf(slotId,activePane.id,modal.doc.id):undefined}/>}
