...
function toggleChrome(paneId:string){document.dispatchEvent(new CustomEvent('atlas:reader-chrome',{detail:{paneId}}));setSession(s=>{const p=s.panes.find(p=>p.id===paneId);if(p)p.readerChromeCollapsed=!(p.readerChromeCollapsed??true);});}
    ...
    function navigation(){return <nav className="sidebar-navigation" aria-label="Library navigation">
      <button className="icon-button app-home" title="AtlasNote home" aria-label="AtlasNote home" onClick={()=>{setContextOpen(false);setSession(s=>{s.screen='home';});}}><img src="./assets/atlas.svg" width="20" height="20" alt=""/></button>
      ...
      <IconButton name="context" data-panel-toggle="context" label="Toggle document context" active={contextOpen&&managerMode==='context'} onClick={()=>{if(contextOpen&&managerMode==='context')setContextOpen(false);else openManager('context');}}/>
      ...
      <IconButton name="swap" label="Swap panes" disabled={session.panes.length!==2} onClick={()=>setSession(s=>{s.panes.reverse();s.ratio=100-s.ratio;})}/>
    </nav>;}
    ...
    function managerPanel(){
      ...
      return <ContextPanel resolveAsset={assets.url} catalogue={catalogue} page={session.screen==='reader'?activePage:undefined} view={activeView} location={activeLocation} workspace={ws} paneId={activePane?.id} onOpen={openPage} onJump={jump} onPdfNavigate={pdfNavigate} onPdfToggle={togglePdfTree} onPdfManage={(doc:any)=>setModal({kind:'companion',doc})} onReadingActions={showReadingActions} onClose={()=>setContextOpen(false)} notify={notify}/>;
    }
 ...
 }}><Icon name={p?.cheatsheet?'grid':catalogue.documents.some(d=>d.pageId===p?.id)?'pdf':'page'} size={15}/><span>{p?.title??collection?.title??(route?'Unavailable':'New tab')}</span><button className="tab-close" aria-label={'Close tab '+(p?.title??collection?.title??'new tab')} onClick={e=>{e.stopPropagation();closeTab(pane.id,v.id);}}><Icon name="close" size={13}/></button></div>;
 })}</div>
 <div className="pane-header-actions">
 <IconButton name={sheet||doc?'spread':'openbook'} label={sheet?'Quick cheatsheet Spread':doc?'Quick PDF Spread':'Quick Book mode'} disabled={!loc||!page} active={sheet?loc?.sheetMode==='spread':doc?loc?.pdfMode==='spread':loc?.presentation==='book'} onClick={()=>{if(view)updateView(pane.id,view.id,v=>{if(sheet){const l=current(v)!;l.sheetMode=l.sheetMode==='spread'?'single':'spread';l.scroll=0;}else toggleQuickLayout(v,!!doc);});}}/>
 ...
 onImportCheatsheet={()=>setModal({kind:'cheatsheet-source'})} onPdfManage={()=>{const doc=catalogue.documents.find(d=>d.pageId===activePage?.id);if(doc)setModal({kind:'companion',doc});}} onSettings={settings} onHome={()=>setSession(s=>{s.screen='home';})} onBookmarks={()=>openManager('bookmark')} onEdit={()=>setModal({kind:'edit',page:activePage})} onPrint={()=>setModal({kind:'print'})}
 ...
 {modal?.kind === 'edit' && modal.page && !modal.page.cheatsheet && <EditPageDialog page={modal.page} catalogue={catalogue} onClose={() => setModal(null)} notify={notify}/>}
