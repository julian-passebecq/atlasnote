...
function toggleChrome(paneId:string){document.dispatchEvent(new CustomEvent('atlas:reader-chrome',{detail:{paneId}}));setSession(s=>{const p=s.panes.find(p=>p.id===paneId);if(p)p.readerChromeCollapsed=!(p.readerChromeCollapsed??true);});}
 ...
 <div className="pane-header-actions pane-reader-controls">
 ...
 {active&&<>
 <IconButton name="down" data-panel-toggle="reading" label="Reading mode" disabled={!page||!view||!!page.qcm} aria-haspopup="dialog" aria-expanded={popover==='reading'} onClick={()=>{setContextOpen(false);setPopover(popover==='reading'?null:'reading');}}/>
 </>}
 </div>
 {view&&loc?.collectionId?<CollectionView onEditReference={item=>setModal({kind:'reference-edit',reference:item})} onResource={openTarget} onReferenceActions={showReadingActions} key={view.id+loc.collectionId} id={loc.collectionId} catalogue={catalogue} workspace={ws} onOpen={(id:string,anchor?:Anchor,newTab=false)=>openPage(id,anchor,newTab,pane.id)} onOther={session.panes.length===2?(id:string)=>{const other=session.panes.find(p=>p.id!==pane.id);if(other)openPage(id,undefined,false,other.id);}:undefined} onManage={(item:any)=>setModal({kind:'manage',item})} onBookmark={bookmarkDocument} onCreate={setModalCreate} onImport={(projectId:string,folderId?:string)=>setModal({kind:'settings',projectId,folderId})}/>:
