...
export function PdfStudyTree({doc,workspace,depth,onToggle,onNavigate,onManage,onActions}:Props){
 ...
 {groups.map(c=>{const open=expanded.has(key(c.id)),first=c.rows[0]?.page??1;const categoryTarget:ReadingTarget={kind:'pdf-category',pageId:doc.pageId,documentId:doc.id,...revision,pdfCategoryId:c.id,pdfPage:first};return <div className="pdf-study-category" key={c.id} data-pdf-category={c.id}>
  ...
  {open&&<div className="tree-children">{c.rows.slice(0,limits[c.id]??80).map(row=>{const target:ReadingTarget={kind:'pdf-page',pageId:doc.pageId,documentId:doc.id,...revision,pdfPage:row.page};return <div key={row.id} data-pdf-page={row.page} className="tree-row pdf-study-page" style={{paddingLeft:4+(depth+1)*8}} onContextMenu={e=>onActions?.(target,row.title,e)}>
    ...
    <button className="tree-target" title={row.title} aria-label={'Go to PDF page '+row.page+' in '+doc.title} onClick={e=>jump(row.page,e)} onMouseDown={e=>{if(e.button===1)e.preventDefault();}} onAuxClick={e=>{if(e.button===1)jump(row.page,e);}}><Icon name="page" size={14}/><span>{shortPageTitle(row.title)}</span></button>
    <button className="study-page-link" title={row.title} aria-label={'Open '+row.title+' at PDF page '+row.page} onClick={e=>jump(row.page,e)} onMouseDown={e=>{if(e.button===1)e.preventDefault();}} onAuxClick={e=>{if(e.button===1)jump(row.page,e);}}>p.{row.page}</button>
