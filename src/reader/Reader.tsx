...
export function NoteReader({ page, view, location, resolveAsset, onAnchor, onAction, onLink, fontSize, layoutKey }: {
 ...
 {bookMode ? <><div className="book-grid" ref={grid}/>{book.error && <div className="book-error" role="alert">{book.error}<button data-action="continuous-block" data-block={lastAnchor.current?.blockId}>Open Continuous</button></div>}<div className="book-measure" ref={measure} aria-hidden="true" {...({inert:''} as any)}><div className="content-unit unit-heading" data-kind="heading" data-block-id={page.id}>{title}</div><ContentUnits page={page} view={view} resolveAsset={resolveAsset} onReady={ready}/>{page.sources.length > 0 && <div className="content-unit unit-sources" data-kind="sources" data-block-id={page.id} data-offset="999999"><footer className="page-sources"><h3>Sources &amp; provenance</h3>{page.sources.map((s, i) => <div key={i}><Source source={s}/></div>)}</footer></div>}</div><div className="book-probe" ref={probe} aria-hidden="true" {...({inert:''} as any)}/></> : <article className="continuous-content">{title}<ContentUnits page={page} view={view} resolveAsset={resolveAsset} onReady={ready}/>{page.sources.length > 0 && <footer className="page-sources"><h3>Sources &amp; provenance</h3>{page.sources.map((s, i) => <div key={i}><Source source={s}/></div>)}</footer>}</article>}
 </div>
