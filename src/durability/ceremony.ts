...
const receipts=new WeakMap<object,{archive:VerifiedArchive;previewHash:string;rawState:unknown;plan:any}>();
export async function selectSavedArchive(event:Event,candidate:VerifiedArchive,workspace:Workspace,built:any,resolveAsset:(key:string)=>Promise<Asset|undefined>,rawState:unknown){
 ...
 const ticket=Object.freeze({});receipts.set(ticket,{archive,previewHash:plan.previewHash,rawState:structuredClone(rawState),plan:structuredClone(plan)});
