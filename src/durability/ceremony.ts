...
const receipts=new WeakMap<object,{archive:VerifiedArchive;previewHash:string}>();
export async function selectSavedArchive(event:Event,candidate:VerifiedArchive,workspace:Workspace,built:any,resolveAsset:(key:string)=>Promise<Asset|undefined>){
 ...
 const ticket=Object.freeze({});receipts.set(ticket,{archive,previewHash:plan.previewHash});
