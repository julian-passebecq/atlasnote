import {requireHistoricalRevision,revisionIdentities,archivedAsset} from '../durability/registry.js';
...
export function createAgentInterface(built:any,backend:WorkspaceStore=store){
 ...
 const api={
  ...
  getStorageDiagnostics(){const ws=backend.state,h=ws.history;return {database:'knowledge-atlas',databaseVersion:3,initialized:h?.meta.initialized??false,epoch:h?.meta.epoch??0,revisions:h?.revisions.length??0,resources:h?.heads.length??0,reviews:h?.reviews.length??0,historyBytes:new TextEncoder().encode(JSON.stringify(h??{})).length,localAssets:ws.assets.length,localAssetBytes:ws.assets.reduce((sum,a)=>sum+a.bytes.length,0),limits:HISTORY_LIMITS,backupSchemasRead:[2,3,4,5],backupSchemaWrite:5,archiveCount:h?.archives?.length??0,archivedRevisions:h?.archives?.reduce((n,a)=>n+a.counts.revisions,0)??0,saving:backend.saving,storageError:backend.error||null};},
