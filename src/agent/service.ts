...
const handlers:Record<AgentOperation['kind'],(ctx:ApplyContext)=>void>={
 ...
 'resource.draft':()=>{},
 'resource.restoreAsNewRevision':({ws,c,op})=>{const r=ws.history?.revisions.find(r=>r.revisionId===(op as any).payload.revisionId);if(!r||r.resourceKey!==op.resourceKey)throw Error('Restore revision belongs to another resource');resourceAdapters[r.resourceType].project(ws.overlays,c,r.snapshot);},
...
export function createAgentInterface(built:any,backend:WorkspaceStore=store){
 ...
 const api={
  ...
  getStorageDiagnostics(){const ws=backend.state,h=ws.history;return {database:'knowledge-atlas',databaseVersion:3,initialized:h?.meta.initialized??false,epoch:h?.meta.epoch??0,revisions:h?.revisions.length??0,resources:h?.heads.length??0,reviews:h?.reviews.length??0,historyBytes:new TextEncoder().encode(JSON.stringify(h??{})).length,localAssets:ws.assets.length,localAssetBytes:ws.assets.reduce((sum,a)=>sum+a.bytes.length,0),limits:HISTORY_LIMITS,backupSchemasRead:[2,3,4],backupSchemaWrite:4,saving:backend.saving,storageError:backend.error||null};},
  ...
  async stage(input:unknown){await backend.flush();const plan=validateChangeSet(input);api.preview(plan);return backend.reviewedMutation(ws=>{planChangeSet(built,ws,plan);if(!ws.history)throw Error('History initialization incomplete');if(ws.history.reviews.some(r=>r.id===plan.id))throw Error('ChangeSet ID was already imported');if(ws.history.reviews.length>=HISTORY_LIMITS.reviewRecords||ws.history.reviews.filter(r=>r.status==='staged').length>=HISTORY_LIMITS.drafts)throw Error('Agent review history is full. Export/manage history; no review was pruned.');ws.history.reviews.push({kind:'review',schemaVersion:1,id:plan.id,status:'staged',createdAt:Date.now(),plan});},{source:'ai',summary:'Stage reviewed proposal'});},
