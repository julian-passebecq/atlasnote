...
export function PdfIntake({built,catalogue:c,initialProject,initialFolder,onOpen,onClose,notify,disabled}:any){
 ...
 async function read(file:File){setBusy(true);setError('');setDuplicate(null);setFile(null);try{if(file.size>PDF_MAX_BYTES)throw Error('PDF exceeds the unchanged 20 MiB asset limit. Prepare it offline or keep it as an external reference.');await preflightStorage(file.size);const bytes=new Uint8Array(await file.arrayBuffer());assertSafeAsset('local.pdf',bytes,'application/pdf');const hash=await sha256(bytes),found=compose(built,store.state).documents.find(d=>d.sha256===hash);if(found){setDuplicate(found);return;}setFile({bytes,hash});setMeta({...emptyPdfMetadata(),title:file.name.replace(/\.pdf$/i,'').slice(0,240)});}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
 async function commit(){setBusy(true);setError('');try{await store.flush();const result=await prepareLocalPdf(compose(built,store.state),store.state,file.bytes,meta,pid,parent,newFolder);if(result.duplicate){setDuplicate(result.duplicate);setFile(null);return;}await store.importPacks([], [result.asset!],result.overlays!);store.personal(p=>{for(const id of [pid,parent,result.parentId].filter(Boolean) as string[])if(!activeSession(p).expanded.includes(id))activeSession(p).expanded.push(id);});onClose();onOpen(result.page!.id);notify('Private PDF imported with its exact original bytes and one canonical tree placement.');}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
