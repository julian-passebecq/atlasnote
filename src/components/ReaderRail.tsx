...
export function ReaderRail({pdfRenderer,session,page,view,location:loc,doc,leftVisible,contextOpen,popover,setPopover,onTree,onFocus,onContext,onCompare,onSwap,onBookmark,onTheme,onView,onSettings,onHome,onBookmarks,onEdit,onPrint,onFlags,onRating,rating}:any){
 ...
 {popover==='more'&&<div className="more-options">
  ...
  <hr/><button onClick={()=>choose(onSettings)}><Icon name="settings"/>Workspace settings</button><small className="secondary">AtlasNote 1.2.1 <span aria-hidden="true">/</span> Local-first workspace</small>
 </div>}
 </FloatingPanel>}
 </>;
}
