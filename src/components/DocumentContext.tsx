...
export function ContextPanel({catalogue:c,page,location,workspace:ws,paneId,onOpen,onJump,onClose,notify,onPdfNavigate,onPdfToggle,onPdfManage,onReadingActions}:any){
  ...
  {page?<div className="context-scroll" id={'context-body-'+tab} role="tabpanel" aria-labelledby={'context-tab-'+tab}>
   {tab==='outline'&&<>
    {doc?<PdfStudyTree doc={doc} workspace={ws} depth={0} onToggle={onPdfToggle} onNavigate={onPdfNavigate} onManage={onPdfManage} onActions={onReadingActions}/>:<>
     {outline.map(x=><button key={x.id} className="outline-link" style={{paddingLeft:8+x.depth*10}} onClick={()=>onJump(x.id)}>{x.title}</button>)}
   ...
   {tab==='search'&&<>
    ...
    {query.trim()&&<>
     {blockHits.map(hit=><button key={hit.id} className="document-search-hit" onClick={()=>onJump(hit.id)}><strong>{hit.title}</strong><span>{hit.excerpt}</span></button>)}
   ...
   {tab==='history'&&<>
    <dl className="document-history"><dt>First opened on this device</dt><dd>{time(visit?.firstOpenedAt)}</dd><dt>Last opened</dt><dd>{time(visit?.lastOpenedAt)}</dd><dt>Added date / source modification</dt><dd>Not recorded by this source.</dd>{note&&<><dt>Last remark edit</dt><dd>{time(note.updatedAt)}</dd></>}</dl>
