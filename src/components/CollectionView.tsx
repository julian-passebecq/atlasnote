...
export function CollectionView({id,catalogue:source,workspace,onEditReference,onResource,onReferenceActions,onOpen,onOther,onManage,onBookmark,onCreate,onImport}: {id:string;catalogue:Catalogue;workspace:Workspace;onEditReference?:any;onResource?:any;onReferenceActions?:any;onOpen:any;onOther?:any;onManage:any;onBookmark:any;onCreate:any;onImport:any}){
 const mode=activeSession(workspace.personal).libraryMode??'notes';const catalogue=useMemo(()=>mode==='notes'?{...source,projects:projectLibrary(source,workspace.overlays,'notes')}:source,[source,workspace.overlays,mode]);
 ...
 function show(e:any,item:CollectionItem){if(item.node.target&&onReferenceActions){onReferenceActions(item.node.target,item.node.title,e);return;}e.preventDefault();e.stopPropagation();const el=e.currentTarget as HTMLElement,r=el.getBoundingClientRect();setMenu({item,x:e.clientX||r.left,y:e.clientY||r.bottom,returnFocus:(e.target instanceof Element?(e.target as Element).closest<HTMLButtonElement>('button'):null)??(el.matches('button')?el:el.querySelector<HTMLButtonElement>('.collection-open'))??el});}
 function choose(fn:()=>void){dismiss();fn();}
 function open(item:CollectionItem,newTab=false){if(item.node.target&&onResource){onResource(item.node.target,newTab?'tab':'here');return;}onOpen(item.page?.id??item.node.id,undefined,newTab);}
 ...
 return <main className="collection-view" aria-label={'Collection: '+target.title}>
  ...
  {!target.project.id.startsWith('project.unfiled.')&&<div className="collection-actions"><IconButton name="settings" label={target.folder?'Manage folder':'Manage notebook'} onClick={()=>onManage({project:target.project,node:target.folder})}/><button onClick={()=>onCreate('page',target.project,target.folder)}><Icon name="plus"/>New note</button><button onClick={()=>onCreate('folder',target.project,target.folder)}><Icon name="folder"/>New folder</button><button onClick={()=>onImport(target.project.id,target.folder?.id)}><Icon name="pdf"/>Import PDF privately</button></div>}
  <section className="collection-filters" aria-label="Filter collection">
   <div className="collection-type-filter" aria-label="Document type">{(pdfMode?[['all','PDF Library']]:[['all','All'],['note','Notes'],['pdf','PDFs'],['reference','References']]).map(([value,label])=><button key={value} aria-pressed={filter.type===value} className={filter.type===value?'selected':''} onClick={()=>change('type',value)}>{label}</button>)}</div>
  ...
  <div className="collection-grid">{shown.map(item=>{const title=item.node.target?item.node.title:item.page?.title??item.node.title;return <article className="collection-card" key={item.node.id} data-item-type={item.type} data-node-id={item.node.id} onContextMenu={e=>show(e,item)} onKeyDown={e=>{if(e.key==='ContextMenu'||e.shiftKey&&e.key==='F10')show(e,item);}}>
   <div className="collection-card-top"><span className={'collection-type-icon type-'+item.type}><Icon name={item.type==='reference'?'link':item.type==='note'?'page':item.type} size={21}/></span><span className="badge">{item.type==='pdf'?'PDF':item.type==='note'?'Note':item.type==='reference'?'Reference':'Folder'}</span>{item.node.target&&<IconButton name="edit" label={'Edit reference '+title} onClick={()=>onEditReference?.(workspace.overlays.references?.find(r=>r.id===item.node.id))}/>}<IconButton name="more" label={'Collection actions for '+title} aria-haspopup="menu" onClick={e=>show(e,item)}/></div>
