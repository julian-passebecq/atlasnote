...
export type Anchor={blockId?:string;offset?:number;pdfPage?:number;pdfRevision?:string};
export type Location={pageId:string;anchor?:Anchor;scroll?:number;presentation:'continuous'|'book'|'parallel';pdfMode:'single'|'continuous'|'spread';pdfPage:number;zoom:number;rotation:number;cover:boolean};
...
export type Session={panes:Pane[];activePane:string;ratio:number;screen:'home'|'reader'|'bookmarks';leftOpen:boolean;rightOpen:boolean;focus:boolean;theme:'fluent'|'neutral'|'academic';showFlags:boolean;expanded:string[];fontSize:number};
...
export const THEME_LABELS={fluent:'Microsoft Fluent',neutral:'Consulting Neutral',academic:'Academic Minimal'};
