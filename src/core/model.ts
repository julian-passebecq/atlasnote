...
export type Location={previousPresentation?:'continuous'|'parallel';previousPdfMode?:'single'|'continuous';pageId:string;collectionId?:string;anchor?:Anchor;scroll?:number;presentation:'continuous'|'book'|'parallel';pdfMode:'single'|'continuous'|'spread';pdfPage:number;zoom:number;rotation:number;cover:boolean};
...
export type Session={categoryFilter?:CategoryId|null;compactTop?:boolean;collapsedPane?:string|null;collapsedGroups?:string[];panes:Pane[];activePane:string;ratio:number;screen:'home'|'reader'|'bookmarks';leftOpen:boolean;rightOpen:boolean;focus:boolean;theme:'fluent'|'neutral'|'academic'|'lavender'|'slate';libraryMode?:'notes'|'pdfs';showFlags:boolean;expanded:string[];fontSize:number};
...
export type Personal={schemaVersion:2|3;activeWorkspaceSlot?:WorkspaceNumber;workspaceSlots?:Partial<Record<2|3|4|5,Session>>;notes:Record<string,Remark>;ratings:Record<string,'gray'|'red'|'orange'|'green'>;bookmarks:Bookmark[];session:Session};
...
export type Overlays={schemaVersion:2;companions?:Record<string,import('../companion/model.js').PdfCompanion>;glossary?:Term[];categories?:Record<string,CategoryId|null>;pages:Record<string,{page:Page;baseHash?:string}>;projects:Project[];projectPrefs:Record<string,{title?:string;icon?:string;description?:string;hidden?:boolean;order?:number}>;operations:StructuralOperation[];archived:string[];groups:Group[]|null;documents:DocumentEntry[]};
