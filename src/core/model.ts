...
export type Overlays={assetBindings?:Record<string,Record<string,import('../history/model.js').AssetReference>>;treeSnapshots?:Record<string,Project>;taxonomy?:Record<string,import('../content-hub/model.js').TaxonomyRef|null>;references?:import('../content-hub/model.js').NotebookReference[];schemaVersion:2;companions?:Record<string,import('../companion/model.js').PdfCompanion>;glossary?:Term[];categories?:Record<string,CategoryId|null>;pages:Record<string,{page:Page;baseHash?:string}>;projects:Project[];projectPrefs:Record<string,{title?:string;icon?:string;description?:string;hidden?:boolean;order?:number}>;operations:StructuralOperation[];archived:string[];groups:Group[]|null;documents:DocumentEntry[]};
export type Asset={key:string;bytes:Uint8Array;mediaType:string;sha256:string};
export type Workspace={viewHistoryRevisionId?:string;history?:import('../history/model.js').HistoryData;imports:Pack[];overlays:Overlays;personal:Personal;assets:Asset[];generation:number};
