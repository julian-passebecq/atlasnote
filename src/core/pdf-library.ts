...
export async function prepareLocalPdf(c:Catalogue,ws:Workspace,bytes:Uint8Array,meta:PdfMetadata,projectId:string,parentId='',newFolder=''){
 ...
 // A remote metadata reference is not a local byte copy. Explicit local intake
 // may create a private copy, while real local/bundled duplicates remain blocked.
 const hash=await sha256(bytes),duplicate=c.documents.find(d=>d.sha256===hash&&!!d.assetKey);
...
export function duplicatePdfImports(c:Catalogue,incoming:any[]){
 const seen=new Map(c.documents.filter(d=>d.sha256&&d.assetKey).map(d=>[d.sha256!,d]));const duplicates:string[]=[];
 for(const d of incoming.flatMap(p=>p.documents??[])){
  if(!d.sha256||!d.assetKey)continue;const old=seen.get(d.sha256);
