import {assertHistoryTarget,REFERENCES_PROJECT} from '../history/adapters.js';
import {walkBlocks} from './validation.mjs';
import {readingTargetId,normalizeReadingTargetIdentity} from './reading-types.js';
...
export function openReadingTarget(p:Personal,c:Catalogue,o:Overlays,target:ReadingTarget,destination:ReadingDestination,workspace?:import('./model.js').Workspace):void{
 if(!['here','tab','pane',1,2,3,4,5].includes(destination))throw Error('Invalid ReadingDestination; use here, tab, pane or workspace 1-5.');
 validateReadingTarget(target);if(target.historyRevisionId){if(!workspace)throw Error('Historical navigation requires the workspace history.');c=assertHistoryTarget(c,workspace,target);o={...o,archived:[]};}target=normalizeReadingTargetIdentity(c,target);if(target.kind==='url')throw Error('External addresses open only through an explicit link.');
 ...
 if(target.kind==='collection'){if(id!==REFERENCES_PROJECT&&!c.projects.some(x=>x.id===id)&&!findNode(c.projects,id))throw Error('The saved collection is unavailable. Import its library first.');}
 ...
 // A pin means this exact source, not the current section or a parent fallback.
 if(target.historyRevisionId&&page){
  const anchor='anchor' in target?target.anchor:undefined;
  if(sheet&&anchor?.blockId&&sheetPosition(sheet,{anchor,...(target.kind==='cheatsheet-page'?{sheetPage:target.sheetPage}:{})}).anchorMissing)throw Error('This block is not present in the selected historical cheatsheet. The pin was retained.');
  if(!sheet&&!doc&&anchor?.blockId&&anchor.blockId!==page.id){let found=false;walkBlocks(page.blocks,(block:any)=>{if(block.id===anchor.blockId)found=true;});if(!found)throw Error('This section is not present in the selected historical version. The pin was retained.');}
  if(page.qcm&&anchor?.questionId&&!page.qcm.questions.some(q=>q.id===anchor.questionId))throw Error('This question is not present in the selected historical version.');
  if(doc&&anchor?.pdfPage&&doc.pageCount&&anchor.pdfPage>doc.pageCount)throw Error('This physical page is not present in the selected historical PDF.');
 }
