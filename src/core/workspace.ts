import {cheatsheetText,blockText as sheetBlockText} from '../cheatsheets/text-layout.mjs';
...
export function isArchived(pageId:string,c:Catalogue,overlays:Overlays,locs?:ReturnType<typeof locations>){const loc=(locs??locations(c)).get(pageId);return overlays.archived.includes(pageId)||!!loc&&[loc.nodeId,...loc.ancestors].some(id=>overlays.archived.includes(id));}
export function pageText(page:Page,opts={english:true,answers:true}):string{return page.qcm?[page.title,...page.qcm.questions.map(q=>[q.prompt,...q.options.map(o=>o.text+' '+(o.explanation??'')),q.explanation,q.followUp].filter(Boolean).join('\n'))].join('\n'):page.kind==='cheatsheet'&&page.cheatsheet?cheatsheetText(page.cheatsheet):blocksText(page.blocks,opts);}
export function blocksText(blocks:any[],opts={english:true,answers:true}):string{return blocks.map(b=>{switch(b.type){case 'section':return b.title+'\n'+blocksText(b.children,opts);case 'markdown':return b.text;case 'callout':return b.title+'\n'+b.text;case 'list':return b.items.join('\n');case 'table':return [b.columns,...b.rows].map(x=>x.join(' | ')).join('\n');case 'code':return [b.title,b.code,b.explanation,b.output].filter(Boolean).join('\n');case 'bilingual':return b.no+(opts.english?'\n'+b.en:'')+(b.hint?'\n'+b.hint:'');case 'question':return b.question+(opts.answers?'\n'+b.answer+(b.followUp?'\n'+b.followUp:''):'\n[Answer omitted]');case 'image':case 'diagram':return b.caption+'\n'+(b.code??'')+'\nSource: '+b.source.title+(b.source.url?' '+b.source.url:'');case 'resource-link':return b.label+' ['+b.target.kind+']'+(b.description?' '+b.description:'');case 'link':return `${b.label??b.pageId} [${b.pageId}]`;default:return '';}}).join('\n\n');}
export function normalize(s:string){return s.toLocaleLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');}
/** V3 search index: normalized text is computed once per immutable Page/term
 * object (WeakMap), so typing re-scans cached strings instead of re-normalizing
 * every page and every block per keystroke. An Experience-filtered subset reuses
 * the same entries. Block anchors are resolved only for pages that match.
 * Results are identical to the previous implementation (tests/v3-search). */
type PageSearchEntry={text:string;title:string;sheet:[string,string][];blocks:[string,string][]};
const pageSearchCache=new WeakMap<Page,PageSearchEntry>(),termSearchCache=new WeakMap<object,string>();
function pageSearchEntry(page:Page):PageSearchEntry{
 let e=pageSearchCache.get(page);if(e)return e;
 const sheet:[string,string][]=[],blocks:[string,string][]=[];
 if(page.cheatsheet)for(const s of page.cheatsheet.pages)visitCheatsheetBlocks(s.blocks,(b:any)=>{sheet.push([b.id,normalize(sheetBlockText(b))]);});
 walkBlocks(page.blocks,(b:any)=>{blocks.push([b.id,normalize(blocksText([b]))]);});
 e={text:normalize([page.title,page.summary,page.tags.join(' '),pageText(page)].join(' ')),title:normalize(page.title),sheet,blocks};pageSearchCache.set(page,e);return e;
}
export function searchCatalog(c:Catalogue,query:string){const words=normalize(query).trim().split(/\s+/).filter(Boolean);if(!words.length)return {pages:[],terms:[]};const locs=locations(c);
 const hits=c.pages.map(page=>{const e=pageSearchEntry(page),path=normalize(locs.get(page.id)?.path.join(' ')??''),has=(w:string)=>e.text.includes(w)||path.includes(w);
  const score=words.every(has)?words.reduce((s,w)=>s+(e.title.includes(w)?10:1),0):0;let anchor:string|undefined;
  if(score>0){for(const [id,text] of e.sheet)if(!anchor&&words.every(w=>text.includes(w)))anchor=id;for(const [id,text] of e.blocks)if(!anchor&&words.every(w=>text.includes(w)))anchor=id;}
  return {page,score,anchor};}).filter(p=>p.score>0).sort((a,b)=>b.score-a.score);
 const terms=c.glossary.filter(t=>{let text=termSearchCache.get(t);if(text===undefined){text=normalize([t.label,t.definition,t.translation,t.example,...(t.aliases??[]),...(t.tags??[])].filter(Boolean).join(' '));termSearchCache.set(t,text);}return words.every(w=>text!.includes(w));});return {pages:hits,terms};}
...
/** Compare is a second workspace, never a clone of the current document.
...
export function toggleCompare(session:Session):boolean {
 ...
 const pane={id:uid('pane'),views:[],active:''};
 session.panes.push(pane);session.activePane=pane.id;session.screen='reader';return true;
}
