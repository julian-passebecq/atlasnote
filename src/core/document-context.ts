...
export function documentBlockMatches(page:Page,query:string):{id:string;title:string;excerpt:string}[]{
 ...
 if(page.qcm)return page.qcm.questions.flatMap((q,i)=>{const text=[q.prompt,...q.options.map(o=>o.text+' '+(o.explanation??'')),q.explanation??''].join(' '),at=text.toLocaleLowerCase().indexOf(needle);return at>=0?[{id:q.id,title:'Question '+(i+1),excerpt:text.slice(Math.max(0,at-45),at+needle.length+130)}]:[];}).slice(0,80);
 if(page.cheatsheet){for(const [i,sheet] of page.cheatsheet.pages.entries())visitCheatsheetBlocks(sheet.blocks,(b:any)=>{const text=sheetBlockText(b),at=text.toLocaleLowerCase().indexOf(needle);if(at>=0)hits.push({id:b.id,title:'p.'+(i+1)+' - '+sheet.title,excerpt:text.slice(Math.max(0,at-45),at+needle.length+130)});});return hits.slice(0,80);}
